...
public class IntegrationTest : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    [Trait("Category", "Meus Testes")]
    [Theory(DisplayName = "Teste da Rota Post de City")]
    [InlineData("/city")]
    public async Task TestPost(string url)
    {
        var response = await _clientTest.PostAsync(url, new StringContent(JsonConvert.SerializeObject(new City { CityId = 3, Name = "Juiz de Fora" }), System.Text.Encoding.UTF8, "application/json"));
        Assert.Equal(System.Net.HttpStatusCode.Created, response?.StatusCode);
    }


    [Trait("Category", "Meus testes")]
    [Theory(DisplayName = "Teste da Rota Post de Hotel")]
    [InlineData("/hotel")]
